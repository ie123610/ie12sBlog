...
当时最新版本的DLP文件应为 v44：[下载链接](htthttp://prdownloads.sourceforge.net/emulextreme/antileech-sources.rar?downloadp://)  
...
* [英文版wiki](http://https://wiki.bitcomet.com/known_client_issues_and_incompatibilities?s[]=padding#what_are_these_padding_file_things_in_the_filelist)  
...
其以单文件为单位进行传输 没有BT中任务的概念  
由于不存在任务概念 其不受BT任务停止和运行的影响  
于eMule中的共享列表相似 若文件存在则始终可以进行上传  

同时由于以单文件进行传输 可以跨越BT中的任务/种子  
在BT中及时两个任务包含有一个或多个相同的文件 它们直接也不能互相传输  
因为其所在的任务/种子不同  
...
这使得其从公共BT网络汲取的流量只在迅雷客户端直接  
进行流转 形成了一个私有流量池 无法反哺其他非迅雷客户端  
...
**这就意味着若一个BC客户端没有开放端口 他就无法成为一个可用的长效种子**  
就一点足以刷掉大量的客户端 与此同时**长效上传的优先级是低于BT任务的**  

再考虑到官方协调服务器的稳定性和不同资源客户端分布的情况  
...
如果按照这样的逻辑推导岂不是可以认为 “腾讯收购了奇瑞” 或者 “奇瑞确实是邪恶腾讯的仆从”？  
这完全是没有道理的事情  
...
不过其中的注册公司 又引出了新的疑问 其是外国公司还是国内公司   
这在之后对比特彗星发展的研究中会详细讲解  
