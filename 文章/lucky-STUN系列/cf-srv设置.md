...
DNS更新令牌用于在更新DNS是进行身份验证  
获取方法在之前的DNS解析教程中已经讲解过此处不再赘述  
...
相比起dynv6在CF上获取区域ID会更加容易 在浏览器中即可查看  
...
在获取了区域ID后仍需要获取DNS记录ID  
其对应我们之前创建的SRV记录  
这里和之前的教程一样使 用上lucky的 callweb功能进行获取  
