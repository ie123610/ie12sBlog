...
不过在本教程中 主要是使用其来“隐藏”WEB服务的端口  

**HTTPS是SVCB记录中的一种 专门用于指示HTTPS服务参数**  
在设置/查询记录类型的时候需要注意区分  

不论是SRV记录还是SVCB/HTTPS 记录  
都相当于是一种特殊的TXT记录 用于记录服务参数  
...
与Windows上不同的是 目前安卓版本的火狐没无法自定义DoH服务器  
但其可以使用系统设置中的DoH 设置系统DoH后即可使用HTTPS  
